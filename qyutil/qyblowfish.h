...
//Block Structure
struct QYUTIL_API QyBlock
{
	//Constructors
	QyBlock(unsigned int l=0, unsigned int r=0) : m_uil(l), m_uir(r) {}
	//Copy Constructor
	QyBlock(const QyBlock& roBlock) : m_uil(roBlock.m_uil), m_uir(roBlock.m_uir) {}
	QyBlock& operator^=(QyBlock& b) { m_uil ^= b.m_uil; m_uir ^= b.m_uir; return *this; }
	unsigned int m_uil, m_uir;
};

class QYUTIL_API QyBlowFish
{
public:
	enum { ECB=0, CBC=1, CFB=2 };

	//Constructor - Initialize the P and S boxes for a given Key
	QyBlowFish(unsigned char* ucKey, size_t n, const QyBlock& roChain = QyBlock(0UL,0UL));
	...
	static void MakeEncrypt(const unsigned char* in,unsigned char* out,size_t n,int iMode = ECB);
	static void MakeDecrypt(const unsigned char* in, unsigned char* out, size_t n, int iMode=ECB);
...
private:
	...
	void Encrypt(QyBlock&);
	void Decrypt(QyBlock&);
...
//Function F
inline unsigned int QyBlowFish::F(unsigned int ui)
{
