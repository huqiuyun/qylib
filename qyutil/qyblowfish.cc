...
const unsigned int QyBlowFish::scm_auiInitP[18] = {
...
const unsigned int QyBlowFish::scm_auiInitS[4][256] = {
	//0
...
#define exception_blowfish std::exception 

#else

/**
* @brief exception in blow fish
*
* @author welbon
*
* @date 2010-08-26
*/
class exception_blowfish : public std::exception
{
public :
	exception_blowfish(const char* err) throw()
	{
		m_perr = NULL;
		if (err)
		{
 	...
 	virtual ~exception_blowfish(void) throw()
	{
		delete [] m_perr ;
	}
...
QyBlowFish::QyBlowFish(unsigned char* ucKey, size_t keysize, const QyBlock& roChain) : m_oChain0(roChain), m_oChain(roChain)
{
	if (keysize<1){
		//throw std::exception("Incorrect key length");
		throw exception_blowfish("Incorrect key length");
	}
	...
	//Reflexive Initialization of the Blowfisstd::exceptionh.
	...
	QyBlock block(0UL,0UL); //all-zero block
	for(i=0; i<18;)
...
void QyBlowFish::Encrypt(QyBlock& block)
{
...
//Sixteen Round Decipher of QyBlock
void QyBlowFish::Decrypt(QyBlock& block)
{
	unsigned int uiLeft = block.m_uil;
	unsigned int uiRight = block.m_uir;
	uiLeft ^= m_auiP[17];
...
inline void BytesToBlock(unsigned char const* p, QyBlock& b)
{
	unsigned int y;
	//Left
	b.m_uil = 0;
...
inline void BlockToBytes(QyBlock const& b, unsigned char* p)
{
	unsigned int y;
	//Right
	y = b.m_uir;
...
//Returns false if n is multiple of 8
void QyBlowFish::Decrypt(unsigned char* buf, size_t n, int iMode)
{
	//Check the buffer's length - should be > 0 and multiple of 8
	if ((n==0)||(n%8!=0)){
		//throw std::exception("Incorrect buffer length");
...
//Returns false if n is multiple of 8
void QyBlowFish::Encrypt(const unsigned char* in, unsigned char* out, size_t n, int iMode)
{
	//Check the buffer's length - should be > 0 and multiple of 8
	if ((n==0)||(n%8!=0)){
		//throw std::exception("Incorrect buffer length");
...
//Returns false if n is multiple of 8
void QyBlowFish::Decrypt(const unsigned char* in, unsigned char* out, size_t n, int iMode)
{
	//Check the buffer's length - should be > 0 and multiple of 8
	if ((n==0)||(n%8!=0)){
		//throw std::exception("Incorrect buffer length");
...
void QyBlowFish::Char2Hex(const unsigned char ch, char* szHex)
{
...
void QyBlowFish::Hex2Char(const char* szHex, unsigned char& rch)
{
   rch = 0;
	for(int i=0; i<2; i++)
	{
...
void QyBlowFish::CharStr2HexStr(const unsigned char* pucCharStr, char* pszHexStr, int iSize)
{
	int i;
	char szHex[3];
	pszHexStr[0] = 0;
...
void QyBlowFish::HexStr2CharStr(const char* pszHexStr, unsigned char* pucCharStr, int iSize)
{
   int i;
	unsigned char ch;
	for(i=0; i<iSize; i++)
...
void QyBlowFish::MakeEncrypt(const unsigned char* in,unsigned char* out,size_t n,int iMode)
{
	unsigned char key[] = "DF45%&*@rty6!67";
	QyBlowFish oBlowFish(key, ENCRYPT_KEY_LENGTH);
	oBlowFish.Encrypt(in, out, n);    //注意传入必须是8的倍数
}

void QyBlowFish::MakeDecrypt(const unsigned char* in, unsigned char* out, size_t n, int iMode)
{
	unsigned char key[] = "DF45%&*@rty6!67";
	QyBlowFish oBlowFish(key, ENCRYPT_KEY_LENGTH);
	oBlowFish.Decrypt(in, out, n);
}
