 ...
 */
void
qyrc4_crypt(struct rc4_state *const state,
...
void qyrc4_crypt(const char *inbuf, char *outbuf, size_t buflen, const char *key, size_t keylen)
{
	rc4_state state;

	rc4_init(&state, (u_char *)key, keylen);
	qyrc4_crypt(&state, (const u_char *)inbuf, (u_char *)outbuf, buflen);
}
