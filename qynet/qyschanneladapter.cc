...
namespace qy 
{
		...
		SChannelAdapter::BeginSSL()
    {
		...
		ret = AcquireCredentialsHandle(NULL, UNISP_NAME, SECPKG_CRED_OUTBOUND, NULL,&sc_cred, NULL, NULL, &impl_->cred, NULL);
		if (ret != SEC_E_OK) 
        {
			return ret;
		}
		...
		if(LOG_CHECK_LEVEL(LS_VERBOSE) )
		{
			...
			ret = QueryCredentialsAttributes(&impl_->cred,SECPKG_ATTR_CIPHER_STRENGTHS,&cipher_strengths);
			if (SUCCEEDED(ret)) 
			{
            }

			SecPkgCred_SupportedAlgs supported_algs = { 0 };
			ret = QueryCredentialsAttributes(&impl_->cred,SECPKG_ATTR_SUPPORTED_ALGS,&supported_algs);
			if (SUCCEEDED(ret))
			{
		...
		ret = InitializeSecurityContextA(&impl_->cred, NULL,const_cast<char*>(ssl_host_name_.c_str()),flags, 0, 0, NULL, 0,	&impl_->ctx, sb_out.desc(),	&ret_flags, NULL);
		if (SUCCEEDED(ret))
			impl_->ctx_init = true;
	...
	int	SChannelAdapter::ContinueSSL() 
	{
		...
		CSecBufferBundle<2, CSecBufferBase::FreeSSPI> sb_out;
		ret = InitializeSecurityContextA(&impl_->cred, &impl_->ctx,	const_cast<char*>(ssl_host_name_.c_str()),flags, 0, 0, sb_in.desc(), 0,NULL, sb_out.desc(),	&ret_flags, NULL);
		return ProcessContext(ret, sb_in.desc(), sb_out.desc());
	}

	int	SChannelAdapter::ProcessContext(long int status, _SecBufferDesc* sbd_in,_SecBufferDesc* sbd_out)
	{
		...
		SChannelAdapter::DecryptData()
	{
		...
		while (!inbuf.empty()) {
			...
			if (SUCCEEDED(status)) {
				...
				if (status != SEC_E_OK) {
					LOG(LS_INFO) << "DecryptMessage returned continuation code: "<< ErrorName(status, SECURITY_ERRORS);
				}
				continue;
			}
	...
	void SChannelAdapter::PostEvent() 
	{
		...
		if(CMsgThread* thread = CMsgThread::Current() ) {
	...
	{
		LOG(LS_WARNING) << "SChannelAdapter::Error("<< context << ", "<< ErrorName(err, SECURITY_ERRORS) << ")";
		state_ = SSL_ERROR;
		SetError(err);
		if (signal)
	...
	int	SChannelAdapter::Send(const void* pv, size_t cb) 
	{
		...
		while (written < cb)
		{
			const size_t encrypt_len = std::min<size_t>(cb - written,impl_->sizes.cbMaximumMessage);
	...
	Socket::ConnState SChannelAdapter::GetState() const
	{
		...
		if ((state == CS_CONNECTED) && ((state_ == SSL_WAIT) || (state_ == SSL_CONNECTING)))
			state = CS_CONNECTING;
		return state;
	}
