...
#define MAX_OPT_SIZE    40         // Maximum length of IP options in bytes
	...
	WinPing::PingResult WinPing::Ping(uint32 ip, uint32 data_size, uint32 timeout, uint8 ttl,bool allow_fragments)
	{
		...
		DWORD result = send_(hping_, qy::HostToNetwork32(ip),	data_, uint16(data_size), &ipopt,reply_, reply_size, timeout);
		if (result == 0) 
		{
